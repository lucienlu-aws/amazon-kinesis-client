...
class DynamoDBLeaseCoordinatorTest {
    ...
    @BeforeEach
    void setUp() {
        this.leaseRefresher = new DynamoDBLeaseRefresher(TEST_LEASE_TABLE, dynamoDbAsyncClient,
                new DynamoDBLeaseSerializer(), true, NOOP_TABLE_CREATOR_CALLBACK, Duration.ofSeconds(10),
                new DdbTableConfig(), true, new ArrayList<>());
    }
    ...
    void initialize_withLeaseAssignmentManagerMode_assertIndexOnTable()
        ...
        final DescribeTableResponse response = dynamoDbAsyncClient.describeTable(DescribeTableRequest
                                                                                    .builder()
                                                                                    .tableName(TEST_LEASE_TABLE)
                                                                                    .build()).join();

        assertEquals(1, response.table().globalSecondaryIndexes().size());
        assertEquals(IndexStatus.ACTIVE, response.table().globalSecondaryIndexes().get(0).indexStatus());
    }
    ...
    void initialize_withDefaultMode_assertIndexInCreating() throws ProvisionedThroughputException, DependencyException {
        ...
        assertEquals(1, response.table().globalSecondaryIndexes().size());
        assertEquals(IndexStatus.CREATING, response.table().globalSecondaryIndexes().get(0).indexStatus());
    }

    private void constructCoordinatorAndInitialize()
            throws ProvisionedThroughputException, DependencyException {
        this.dynamoDBLeaseCoordinator = new DynamoDBLeaseCoordinator(leaseRefresher, "Identifier", 100L, true, 100L, 10, 10,
                10, 100L, 100L, new NullMetricsFactory(),
