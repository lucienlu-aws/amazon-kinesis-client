...
public class DynamoDBLockBasedLeaderDecider implements LeaderDecider {
    private static final Long DEFAULT_LEASE_DURATION_MILLIS = Duration.ofMinutes(2).toMillis();
    // Heartbeat frequency should be at-least 3 times smaller the lease duration according to LockClient documentation
    private static final Long DEFAULT_HEARTBEAT_PERIOD_MILLIS = Duration.ofSeconds(30).toMillis();
    ...
    static DynamoDBLockBasedLeaderDecider create(final CoordinatorStateDAO coordinatorStateDao,
        final String workerId, final Long leaseDuration,
        final Long heartbeatPeriod, final MetricsFactory metricsFactory
    ) {
        final AmazonDynamoDBLockClient dynamoDBLockClient = new AmazonDynamoDBLockClient(
            coordinatorStateDao.getDDBLockClientOptionsBuilder()
        ...
        return new DynamoDBLockBasedLeaderDecider(coordinatorStateDao, dynamoDBLockClient,
            heartbeatPeriod, workerId, metricsFactory);
    }

    public static DynamoDBLockBasedLeaderDecider create(final CoordinatorStateDAO coordinatorStateDao,
        final String workerId, final MetricsFactory metricsFactory
    ) {
        return create(coordinatorStateDao, workerId, DEFAULT_LEASE_DURATION_MILLIS,
            DEFAULT_HEARTBEAT_PERIOD_MILLIS, metricsFactory);
    }
    ...
    public synchronized Boolean isLeader(final String workerId) {
        ...
        if (!lockItem.isPresent() || lockItem.get().isExpired()) {
            try {
                ...
                final Optional<LockItem> leaderLockItem = dynamoDBLockClient.tryAcquireLock(
                        AcquireLockOptions.builder(LEADER_HASH_KEY)
    ...
    public void releaseLeadershipIfHeld() {
        try {
            final Optional<LockItem> lockItem = dynamoDBLockClient.getLock(LEADER_HASH_KEY, Optional.empty());
            if (lockItem.isPresent() && !lockItem.get().isExpired() && lockItem.get().getOwnerName().equals(workerId)) {

                log.info("Current worker : {} holds the lock, releasing it.", lockItem.get().getOwnerName());
    ...
    public synchronized boolean isAnyLeaderElected() {
        ...
        if (Duration.between(Instant.ofEpochMilli(lastIsAnyLeaderElectedDDBReadTimeMillis),
                            Instant.ofEpochMilli(System.currentTimeMillis()))
                    .toMillis() > heartbeatPeriodMillis) {
            final MetricsScope metricsScope = MetricsUtil.createMetricsWithOperation(metricsFactory,
                    this.getClass().getSimpleName() + ":isAnyLeaderElected");
            ...
            try {
                ...
                final Optional<LockItem> lockItem = dynamoDBLockClient.getLockFromDynamoDB(
                        GetLockOptions.builder(LEADER_HASH_KEY)
                                      .build());
                ...
                if (!lockItem.isPresent()) {
                    ...
                    if (isNull(lastIsAnyLeaderCheckLockItemToFirstEncounterTime) || !ddbLockItem.getRecordVersionNumber().equals(
                            lastIsAnyLeaderCheckLockItemToFirstEncounterTime.getKey().getRecordVersionNumber())) {
                        ...
                        lastIsAnyLeaderCheckLockItemToFirstEncounterTime = new AbstractMap.SimpleEntry<>(ddbLockItem,
                        ...
                        lastIsAnyLeaderElectedResult = lastIsAnyLeaderCheckLockItemToFirstEncounterTime.getValue()
                                + ddbLockItem.getLeaseDuration() > lastIsAnyLeaderElectedDDBReadTimeMillis;
                ...
                metricsScope.addData("Latency", System.currentTimeMillis() - startTime, StandardUnit.MILLISECONDS,
                        MetricsLevel.DETAILED);
                MetricsUtil.endScope(metricsScope);
            }
        }
