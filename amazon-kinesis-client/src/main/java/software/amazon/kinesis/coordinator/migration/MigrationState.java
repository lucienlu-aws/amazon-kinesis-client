...
public class MigrationState extends CoordinatorState {
    ...
    private MigrationState(final String key, final ClientVersion clientVersion, final String modifiedBy,
        final long modifiedTimestamp, final List<HistoryEntry> historyEntries, final Map<String, AttributeValue> others
    ) {
        setKey(key);
    ...
    public MigrationState(final String key, final String modifiedBy) {
        this(key, ClientVersion.CLIENT_VERSION_INIT, modifiedBy, System.currentTimeMillis(), new ArrayList<>(),
            new HashMap<>());
    }
    ...
    public HashMap<String, AttributeValue> serialize() {
        ...
        if (!history.isEmpty()) {
            ...
            for (final HistoryEntry entry : history) {
                historyList.add(AttributeValue.builder()
                    .m(entry.serialize())
                    .build());
            }
            result.put(HISTORY_ATTRIBUTE_NAME, AttributeValue.builder().l(historyList).build());
        }
    ...
    public static MigrationState deserialize(final String key, final HashMap<String, AttributeValue> attributes) {
        ...
        try {
            ...
            final String modifiedBy = mutableAttributes.remove(MODIFIED_BY_ATTRIBUTE_NAME).s();
            final long modifiedTimestamp = Long.parseLong(mutableAttributes.remove(MODIFIED_TIMESTAMP_ATTRIBUTE_NAME).n());

            final List<HistoryEntry> historyList = new ArrayList<>();
            if (attributes.containsKey(HISTORY_ATTRIBUTE_NAME)) {
                 mutableAttributes.remove(HISTORY_ATTRIBUTE_NAME).l()
                    .stream().map(historyEntry -> HistoryEntry.deserialize(historyEntry.m()))
                    .forEach(historyList::add);
            }
            final MigrationState migrationState = new MigrationState(MIGRATION_HASH_KEY, clientVersion, modifiedBy,
    ...
    public Map<String, ExpectedAttributeValue> getDynamoClientVersionExpectation() {
        return new HashMap<String, ExpectedAttributeValue>() {{
            put(CLIENT_VERSION_ATTRIBUTE_NAME,
    ...
    public MigrationState update(final ClientVersion clientVersion, final String modifiedBy) {
        log.info("Migration state is being updated to {} current state {} caller {}", clientVersion, this,
    ...
    public void addHistoryEntry(final ClientVersion lastClientVersion, final String lastModifiedBy,
    ...
    public Map<String, AttributeValueUpdate> getDynamoUpdate() {
        ...
        updates.put(CLIENT_VERSION_ATTRIBUTE_NAME, AttributeValueUpdate.builder()
            .value(AttributeValue.fromS(clientVersion.name()))
            .action(AttributeAction.PUT)
            .build());
        updates.put(MODIFIED_BY_ATTRIBUTE_NAME, AttributeValueUpdate.builder()
            .value(AttributeValue.fromS(modifiedBy))
            .action(AttributeAction.PUT)
            .build());
        updates.put(MODIFIED_TIMESTAMP_ATTRIBUTE_NAME, AttributeValueUpdate.builder()
            .value(AttributeValue.fromN(String.valueOf(modifiedTimestamp)))
            .action(AttributeAction.PUT)
            .build());
        if (!history.isEmpty()) {
            updates.put(HISTORY_ATTRIBUTE_NAME, AttributeValueUpdate.builder()
                .value(AttributeValue.fromL(history.stream().map(HistoryEntry::toAv).collect(Collectors.toList())))
                .action(AttributeAction.PUT)
                .build());
        }
        return updates;
