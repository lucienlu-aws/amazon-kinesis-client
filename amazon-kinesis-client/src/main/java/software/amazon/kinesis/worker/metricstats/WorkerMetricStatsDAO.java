...
public class WorkerMetricStatsDAO {
    ...
    public WorkerMetricStatsDAO(final DynamoDbAsyncClient dynamoDbAsyncClient, final WorkerMetricsTableConfig tableConfig,
        ...
        this.dynamoDbEnhancedAsyncClient = DynamoDbEnhancedAsyncClient.builder()
                                                                      ...
                                                                      .build();
        this.table = dynamoDbEnhancedAsyncClient.table(tableConfig.tableName(), TableSchema.fromBean(WorkerMetricStats.class));
    ...
    public void updateMetrics(final WorkerMetricStats workerMetrics) {
        ...
        final UpdateItemEnhancedRequest<WorkerMetricStats> request
            = UpdateItemEnhancedRequest.builder(WorkerMetricStats.class)
                .item(workerMetrics)
                .ignoreNulls(true)
                .build();
    ...
    public boolean deleteMetrics(final WorkerMetricStats workerMetrics) {
        ...
        final DeleteItemEnhancedRequest request = DeleteItemEnhancedRequest.builder()
                ...
                .conditionExpression(Expression.builder()
                        ...
                        .expressionValues(ImmutableMap.of(":value",
                                AttributeValue.fromN(Long.toString(workerMetrics.getLastUpdateTime()))))
                        .build())
                .build();
        ...
        try {
            ...
            log.warn("Failed to delete the WorkerMetricStats due to conditional failure for worker : {}", workerMetrics, e);
            return false;
        }
    }
    ...
    private void validateWorkerMetrics(final WorkerMetricStats workerMetrics) {
        ...
        final List<String> entriesWithoutValues = workerMetrics.getMetricStats().entrySet()
                .stream()
        ...
        Preconditions.checkArgument(entriesWithoutValues.isEmpty(),
                "Following metric stats dont have any values " + entriesWithoutValues);
        ...
        Preconditions.checkArgument(
                Duration.between(Instant.ofEpochSecond(workerMetrics.getLastUpdateTime()), Instant.now())
                        .toMillis() < 2 * workerMetricsReporterFrequencyMillis,
            ...
            try (final DynamoDbAsyncWaiter waiter = dynamoDbAsyncClient.waiter()) {
                final WaiterResponse<DescribeTableResponse> response
                    = unwrappingFuture(() -> waiter.waitUntilTableExists(
                        r -> r.tableName(table.tableName()),
                        o -> o.waitTimeout(Duration.ofMinutes(10))));
                response.matched().response().orElseThrow(
                    () -> new DependencyException(
                        new IllegalStateException("Creating WorkerMetricStats table timed out",
    ...
    private Supplier<CompletableFuture<Void>> getWorkerMetricsDynamoTableCreator() {
        ...
        if (tableConfig.billingMode() == BillingMode.PROVISIONED) {
            log.info("Creating worker metric stats table {} in provisioned mode with {}wcu and {}rcu", tableConfig.tableName(),
                tableConfig.writeCapacity(), tableConfig.readCapacity());
            tableCreator = () -> table.createTable(r -> r.provisionedThroughput(
